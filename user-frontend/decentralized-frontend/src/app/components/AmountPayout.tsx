...
const [payingout, setPayingout] = useState(false);
  ...
  useEffect(() => {
    const fetchAmount = async () => {
      ...
      try {
        const balance = await fetch('process.env.NEXT_PUBLIC_API_URLapi/worker/balance', {
          method: 'GET',
          headers: {
            Authorization: token
          }
  ...
  return (
    ...
    <button onClick={ async ()  => {
      setPayingout(true);
     const response = await  fetch('process.env.NEXT_PUBLIC_API_URLapi/worker/payout', {
        method: 'POST',
        headers: {
          Authorization: token
        }
